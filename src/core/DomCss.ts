...
export class DomCss{
	...
	*/
	static fonts(
		...
		Object.keys(data).forEach(family=>{
			const d = typeof(data[family])==='string'?{
				src:"url('"+data[family]+"')"
			...
			.map(([k,v])=>k.replace(/([A-Z])/g, (a, b)=>'-'+b.toLowerCase())+': '+v+';').join('');
	...
	*/
	static rule(
		...
		if(DomCss.flatSelectors.includes(selector)){
			...
			const str=
			Object.keys(data)
			.map(k=>k.replace(/([A-Z])/g, (a, b)=>'-'+b.toLowerCase())+': '+data[k]+';').join('');
				...
				let iterRules=(dat:RRecord<string|number>,tgt:CSSRule)=>{
					for (let name in dat) {
						if(typeof(dat[name])==='object'){
							...
							iterRules(dat[name] as RRecord<string|number>,r.cssRules[(rule as CSSKeyframesRule).cssRules.length-1]);
						}else{
	...
	*/
	static rules(
		...
		let rules:Record<string,CSSStyleRule>={},rdata=this.getRulesData(data);
		if(!(sheet instanceof CSSStyleSheet))sheet = this.sheet;
		
		for(let k in rdata.rules){
			try{
				rules[k]=this.rule(k,rdata.rules[k] as DomRulesDataType,sheet,rules,rdata.alias as Record<string,string|number>);
			}catch(e){
	...
	static rulesText(
		...
		let result=[];
		for(let k in rdata.rules){
			const rr=[],dr = rdata.rules[k] as Record<string,string|number>;
			for(let r in dr){
	...
	*/
	static handleVars(
		...
		const proxy=new Proxy({},{
			get:(tgt,prop:string)=>prop==='setVars'?
			(vars:{[k:string]:string})=>Object.entries(vars).map(([k,v])=>proxy[k]=v)
	...
	*/
	static getRulesData(data:RRecord<string|number>):DomRulesRulesResultType{
		...
		const collect=function(
			...
			let obj:RRecord<string|number>={},rname:string;
			...
			for(let prop in dat){
				...
				}else if(typeof(dat[prop])==='object'){// sub queries
					const s = c==='&'?prop.slice(1):(!pile.length||['>',':'].includes(c)?prop:' '+prop);
	...
	static getValue(node:HTMLElement,name:string){
		if(node.nodeType== 1){
			...
			return (node.style as {[index: string]:any})[name]
