...
import { DomModelRegistry } from "./model/DomModelRegistry";

class DomModuleItem<T> {
	constructor(public data: T, public isPrivate: boolean = false) {}
}
...
export class DomBaseModule {
	...
	import(
		module: DomBaseModule | DomBaseModule[] | DomModelClassType | DomModelClassType[],
		...
		if (module instanceof Array) {
			...
			DomModelRegistry.registerModel(m);
	...
	addModel<TT extends any>(
		...
		if (typeof tagName === "string") {
			class _Model_ extends DomModel {
				...
				public static rulesData:
					| DomRulesDataType
					| ((instance: DomModelClassType) => DomRulesDataType) = cssRules || {};
				constructor(params?: Record<string, any>, children: Array<string | HTMLElement> = []) {
			...
			this.models.set(
				tagName,
				new DomModuleItem<DomModelClassType>(_Model_ as DomModelClassType, isPrivate)
			);
		}
	}
	...
	hasModel(tagName: string, publicOnly: boolean = false): boolean {
		return (
			this.hasOwnModel(tagName, publicOnly) ||
			!!this._modules.find(m => m.data.hasModel(tagName, true))
		);
	}
