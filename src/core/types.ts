...
export type DomRulesType =
	| Record<string, CSSStyleRule>
	| { className: string; rules: Record<string, CSSStyleRule> };

export type DomEvent<TEvent, TTarget> = TEvent & { target: TTarget };
export type DomInputEvent = DomEvent<InputEvent, HTMLInputElement>;

export type DomRulesDataType<T = string | number> = { [k in string]: T | DomRulesDataType<T> } & {
