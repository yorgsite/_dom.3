...
export declare const MemberSignal: <T>() => (target: any, propertyKey: string) => void, signal: <T, TR = T>(value?: T, opt?: Partial<DomSignalOptions<T, TR>>) => DomSignalInterface<T, TR>, effect: (callback: () => void) => DomSignalEffectHandler, computed: <T>(callback: () => T | Promise<T>) => () => T;
export {};
